function [outVar,outVar2] = fine2coarse_grid_tripole(lon,lat,LON,ddeg,gdepth,inVar,inVar2,inlon,inlat,inDepth,inAAA,latlim);

%% [outVar,outVar2] = fine2coarse_grid_tripole(lon,lat,LON,ddeg,gdepth,inVar,inVar2,inlon,inlat,inDepth,inAAA,latlim);
    ...
    for j=1:length(lat)-1
        ...
        if gdepth(j,i)>0 %then ocean; this retains more of the data
            ...
            if lat(j)< latlim
                ...
                % exlclude land and NaNs
                IN = find(isel==1 & inDepth(Iall)>0 & ~isnan(inVar(Iall)));
                numpa(j,i) = length(IN);
                
                % do averaging
                if ~isempty(IN)
                    varTOTa2  = inVar(Iall);
                    varTOTa22 = inVar2(Iall);
                    AAA2      = inAAA(Iall);
                    % area averaging
                    outVar(j,i)  = sum(varTOTa2(IN).*AAA2(IN))/sum(AAA2(IN));
                    outVar2(j,i) = sum(varTOTa22(IN).*AAA2(IN))/sum(AAA2(IN));
