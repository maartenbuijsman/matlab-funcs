...
%  suitable to the Sasha's new s-coordinate with bottom stretching (2009)
%
%  On Input:
%
%    type    'r': rho point 'w': w point
%    scoord     : 'old' (Song, 1994),
%                 'new' (Sasha, 2009), with bottom stretching
...
%
%  modified by Yusuke Uchiyama, UCLA, 2009
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if nargin < 8
  error('Not enough input arguments in my_zlevs_new')
...
if (scoord=='new' | scoord=='NEW');
  ...
  if (theta_b > 0)
    Cs=(exp(theta_b.*(csrf+1.0))-1.0)./(exp(theta_b)-1.0)-1.0;
  else
    Cs=csrf;
  end;
  if (theta_s <= 0 & theta_b <= 0) % uniform spacing
    Cs=sc;
  end
elseif (scoord=='bot' | scoord=='BOT');
  if (theta_b>0);
  x = sc+1.0;
