%% function [xl,yl,sl,varstruct] = get_slice_mitgcm(XC,YC,varstruct,xlp,ylp,nump,getpoints,scfac,DOFF);
%% MCB, GFDL, 2012-03-19
...
% YC=Latitude(Iy,Ix);
% nump=100;
% varstruct(1).var = depth(Iy,Ix);
% varstruct(2).var    = hi(Iy,Ix);
...
[minx,Imin] = min(xlp); [maxx,Imax] = max(xlp);
miny=(ylp(Imin));       maxy=(ylp(Imax));
...
Ix = find(XC(1,:)>min(xlp)-DOFF & XC(1,:)<max(xlp)+DOFF );
Iy = find(YC(:,1)>min(ylp)-DOFF & YC(:,1)<max(ylp)+DOFF );
...
for ii=1:length(varstruct)
    ...
    for i=1:nz2
        ...
        if ~isempty(rem_NaN(varNaN(:)))
            ...
            varstruct(ii).varslice(:,i)  = griddata(XC(Iy,Ix),YC(Iy,Ix),varstruct(ii).var(Iy,Ix,i),xl,yl,'linear');
