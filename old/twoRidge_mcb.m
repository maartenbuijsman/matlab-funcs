function [an,bn,cn,dn,a1,a2,A0,B0,C0,AL,BL,CL,CCA]=twoRidge_mcb(gam0,gamL,knL,dop);
% mcb 2010-10-10
% computes energy between two ridges
...
%% compute energyMCB
om = 2*pi/12.4/3600;
t  = linspace(0,12.4*3600,100);

for jj=1:length(t)
    
  Nx=300;
  for i=1:Nx;
    Ua(:,Nx-i+1) = UU*( an.*exp(sqrt(-1)*knL*(-i)/Nx+sqrt(-1)*om*t(jj)) );
    Ud(:,i)      = UU*( dn.*exp(-sqrt(-1)*knL*(Nx+i)/Nx+sqrt(-1)*om*t(jj)) );
    
    Ub(:,i)      = UU*( bn.*exp(-sqrt(-1)*knL*(i-1)/Nx+sqrt(-1)*om*t(jj)) );
    Uc(:,i)      = UU*( cn.*exp(sqrt(-1)*knL*(i-1)/Nx+sqrt(-1)*om*t(jj)) );
  end;
  
  CC = conv2(real([Ua Ub+Uc Ud]+1),ones(4,1)/4,'same');
  %CCI(jj) = sum(sum(CC(:,301:600))); % inbetween ridges => this number changes as the width changes .....
  CCI(jj) = sum(sum(CC)); % energy in entire domain => stupid, because 
