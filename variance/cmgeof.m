function [varargout]=cmgeof(x,varargin)
% Empirical Orthogonal Function analysis  
% 
% syntax: [ampt,amps,sv,mv] = cmgeof(x, [options]);
%   or    [ampt,amps,sv,mv,eigvec,eigval] = cmgeof(x, [options]);
%           if eigenvector and eigenvalue are desired as output
% 
% Input:
...
if covyes
	...
	if donorm
		if isreal(x)
			z=detrend(x);
			z=(z-repmat(mean(z),m,1))./(ones(m,1)*std(z));
		else
			xr=real(x);
			xi=imag(x);
			zr=detrend(xr);
...
sig = sign(eigvec);     %get the sign of the eigenvector to put into the amplitudes

if covyes
	ampt=nan*ones(1,length(mv));
	fprintf('\nTemporal amplitude can not be computed when the input is a covariance matrix.\n');
else	
...
elseif nargout==6
    ...
    disp('Error: Need to pick either 4 or 6 output variables; see help')
    return
end
